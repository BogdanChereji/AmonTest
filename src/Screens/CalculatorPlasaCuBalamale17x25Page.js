...
function CalculatorPlasaCuBalamalePage() {
  //Template
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `${API_LINK}/api/produsePlasaCuBalamale17x25`,
          {
  ...
  const calculPretTotal = () => {
    if (inaltime && latime) {
      ...
      if (selectedProdus) {
        ...
        if (inaltime * latime <= 0.6) {
          // const reducere = 35; // Procentul de reducere
          // pretTotal = pretTotal - (pretTotal * reducere) / 100; // Scade reducerea din pretTotal
          pretTotal = 10;
        } else if (inaltime * latime > 0.6 && inaltime * latime <= 0.8) {
          pretTotal = 13;
        }
...
export default CalculatorPlasaCuBalamalePage;
