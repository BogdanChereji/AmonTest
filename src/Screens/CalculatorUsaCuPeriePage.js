...
function CalculatorUsaCuPeriePage() {
  //Template
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axios.get(`${API_LINK}/api/produseUsaCuPerie`, {
  ...
  const calculPretTotal = () => {
    if (inaltime && latime) {
      ...
      if (selectedProdus) {
        ...
        if (inaltime * 2 + latime * 2 < 5.96) {
          pretTotal = 66;
        } else if (inaltime * 2 + latime * 2 >= 5.96) {
          pretTotal = 80;
        }
...
export default CalculatorUsaCuPeriePage;
